...
Antes de adentrarnos en la programación de la micro:bit, echemos un vistazo rápido a las bibliotecas y la terminología que serán necesarias para el resto de capítulos.
...
Lo siguiente que vamos a examinar es una pieza central de software en el mundo de Rust Embebido: [`embedded-hal`]. Como su nombre sugiere, se relaciona con el segundo nivel de abstracción que acabamos de conocer: los HALs. La idea detrás de [`embedded-hal`] es proporcionar un conjunto de traits que describen funciones compartidas entre todas las implementaciones de un periférico en todos los HALs. Por ejemplo, podríamos necesitar encender o apagar un pin: independientemete de la placa que sea.

[`embedded-hal`] nos permite escribir controladores independientes para del hardware por ejemplo, un sensor de temperatura, pero que pueda ser utilizado en cualquier chip en el que exista una implementación de los traits de [`embedded-hal`]. Esto se logra programando el controlador de tal manera que solo dependa de los traits de [`embedded-hal`]. Los controladores escritos de esta manera se llaman *independientes de la plataforma.* Afortunadamente, los controladores que obtendremos de `crates.io` son casi todos así.
...
Si nos apetece aprender más sobre estas capas de abstracción, Franz Skarman (a.k.a. [TheZoq2]) dio una charla sobre este tema durante la Oxidize 2020: [An Overview of the Embedded Rust Ecosystem].
