...
`OUT` no es el único registro que puede controlar los pines del puerto `P0`. El registro `OUTSET` también permite cambiar el valor de los pines, al igual que `OUTCLR`. Sin embargo, `OUTSET` y `OUTCLR` no pueden recuperar el estado de salida del puerto `P0`.
...
Estudiemos el programa de abajo. La clave de este programa es `fn print_out`. Esta función imprime el valor actual en `OUT` a la consola `RTT` (`examples/spooky.rs`):
...
Si ejecutamos el programa, veremos algo como esto en la consola `RTT`:
